<?php
/**
 * CakePHP(tm) : Rapid Development Framework (http://cakephp.org)
 * Copyright (c) Cake Software Foundation, Inc. (http://cakefoundation.org)
 *
 * Licensed under The MIT License
 * For full copyright and license information, please see the LICENSE.txt
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright Copyright (c) Cake Software Foundation, Inc. (http://cakefoundation.org)
 * @link      http://cakephp.org CakePHP(tm) Project
 * @since     0.2.9
 * @license   http://www.opensource.org/licenses/mit-license.php MIT License
 */
namespace App\Controller;

use Cake\Core\Configure;
use Cake\Network\Exception\NotFoundException;
use Cake\View\Exception\MissingTemplateException;

/**
 * Static content controller
 *
 * This controller will render views from Template/Pages/
 *
 * @link http://book.cakephp.org/3.0/en/controllers/pages-controller.html
 */
class PagesController extends AppController
{

    /**
     * Displays a view
     *
     * @return void|\Cake\Network\Response
     * @throws \Cake\Network\Exception\NotFoundException When the view file could not
     *   be found or \Cake\View\Exception\MissingTemplateException in debug mode.
     */
    public function display()
    {
        $path = func_get_args();

        $count = count($path);
        if (!$count) {
            return $this->redirect('/');
        }
        $page = $subpage = null;

        if (!empty($path[0])) {
            $page = $path[0];
        }
        if (!empty($path[1])) {
            $subpage = $path[1];
        }

        //get data
        if($page == 'home'){
            
            //get sales
            $start_date = date('Y-m-d', mktime(0, 0, 0, date('m'), 1, date('Y')));
            $end_date = date('Y-m-d', mktime(0, 0, 0, date('m') + 1, 10, date('Y')));

            $this->paginate = [
                'contain' => ['Items', 'Clients'],
                'conditions' => [
                    'Sales.item_extend_end_date >=' => '' . $start_date . '',
                    'Sales.item_extend_end_date <=' => '' . $end_date . '',
                ],
                'order' => ['Sales.item_extend_end_date' => 'DESC'],
            ];

            $sales = $this->paginate($this->Sales);            

            //get invoices
            $this->paginate = [
                'contain' => ['Clients'],
                'order' => ['Invoices.balance_flg' => 'DESC'],
                'limit' => 5
            ];
            
            $invoices = $this->paginate($this->Invoices);

            //get cash receipts
            $this->paginate = [
                'contain' => ['Clients'],
                'order' => ['CashReceipts.balance_flg' => 'DESC'],
                'limit' => 5
            ];
            
            $cashReceipts = $this->paginate($this->CashReceipts);

            //get tasks
            $this->paginate = [
                'contain' => ['Clients', 'MarketResources', 'Users'],
                'order' => ['MarketingTasks.appointment_date' => 'DESC'],
                'limit' => 5
            ];

            $marketingTasks = $this->paginate($this->MarketingTasks);

            //result all
            $this->set(compact('sales', 'invoices', 'cashReceipts', 'marketingTasks'));

        }

        $this->set(compact('page', 'subpage'));

        try {
            $this->render(implode('/', $path));
        } catch (MissingTemplateException $e) {
            if (Configure::read('debug')) {
                throw $e;
            }
            throw new NotFoundException();
        }
    }

    public function isAuthorized($user)
    {        
        // All other actions require an id.
        if (empty($this->request->params['pass'][0])) {
            return false;
        }

        return true;
        
        return parent::isAuthorized($user);
    }
}
